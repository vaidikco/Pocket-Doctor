/*
 * PocketDoctor v4.8.2 – AI-Powered Diagnostic & Autonomous SOS Assistant
...
String createSOSEmailHTML(int level, const Profile& p, const String& ip, unsigned long uptimeSecs) {
  ...
  html += "body{font-family:'Segoe UI',sans-serif;background-color:#F4F6F8;color:#1E293B;padding:24px;line-height:1.6}\r\n";
  ...
  html += "<div class='header'><div class='logo-row'><span style='font-size:28px;'>🚨</span><div class='title'>EMERGENCY SOS ALERT</div></div><div class='subtitle'>PocketDoctor Mark 4 Autonomous Clinical Triage</div></div>\r\n";
...
void dispatchSOSAlert() {
  ...
  if (!readSmtpResponse(client, "250", response)) {
    Serial.println("MAIL FROM Rejected!");
    client.stop();
    return;
  }
  ...
  if (!readSmtpResponse(client, "250", response)) {
    Serial.println("RCPT TO Rejected!");
    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 20);
    display.println("SOS Send Error!");
    display.println("Recipient Rejected");
    display.display();
    client.stop();
    delay(3500);
    return;
  ...
  client.println("DATA");
  if (!readSmtpResponse(client, "354", response)) {
...
void connectWiFi() {
  ...
  String ssid, pass;
  if (wifiConfigValid && strlen(wifiConfig.ssid) > 0) {
