/*
 * PocketDoctor v4.8.2.3 – AI-Powered Diagnostic & Autonomous SOS Assistant (SoftAP Enabled)
 ...
 * - Emergency SOS Mode: Joystick severity selection (1–10) & Rich HTML SMTP Email Dispatch (WiFiClientSecure)
 */
...
// -------------------- DEFAULT SOS & SMTP CONFIG --------------------
const char* default_sos_email = "";
const char* default_smtp_server = "smtp.gmail.com";
const uint16_t default_smtp_port = 465;
const char* default_smtp_user = "";
const char* default_smtp_pass = "";
...
#define CONFIG_MAGIC  0xABCF
#define PROFILES_START 340
...
String base64Encode(const String& input) {
  ...
  for (unsigned char c : input) {
    ...
    valb += 8;
    while (valb >= 0) {
      encoded += b64_alphabet[(val >> valb) & 0x3F];
      valb -= 6;
    }
  }
  if (valb > -6) encoded += b64_alphabet[((val << 8) >> (valb + 8)) & 0x3F];
  while (encoded.length() % 4) encoded += '=';
  return encoded;
}

// Forward declaration & Helper to read SMTP server responses over SSL
bool readSmtpResponse(WiFiClientSecure& client, const char* expectedCode, String& outResponse) {
  unsigned long timeout = millis();
  outResponse = "";
  while (!client.available()) {
    if (millis() - timeout > 12000) {
      Serial.println("SMTP Timeout waiting for response");
      return false;
    }
    delay(10);
  }
  ...
  Serial.print("SMTP Response: ");
  Serial.print(outResponse);
  if (expectedCode != nullptr && strlen(expectedCode) > 0) {
    if (outResponse.indexOf(expectedCode) == -1) {
      return false;
    }
  }
  return true;
...
// -------------------- RICH HTML EMAIL TEMPLATE GENERATOR --------------------
String createSOSEmailHTML(int level, const Profile& p, const String& ip, unsigned long uptimeSecs) {
  String severityColor = (level >= 7) ? "#DC2626" : ((level >= 4) ? "#D97706" : "#2563EB");
  String severityBadgeBg = (level >= 7) ? "rgba(220,38,38,0.12)" : ((level >= 4) ? "rgba(217,119,6,0.12)" : "rgba(37,99,235,0.12)");
  ...
  html += "<title>URGENT SOS MEDICAL ALERT</title>";
  html += "<style>";
  html += "*{margin:0;padding:0;box-sizing:border-box}";
  html += "body{font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,Helvetica,Arial,sans-serif;background-color:#F4F6F8;color:#1E293B;padding:24px;line-height:1.6}";
  html += ".container{max-width:600px;margin:0 auto;background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);border:1px solid #E2E8F0}";
  html += ".header{background:linear-gradient(135deg,#6B0F1A 0%,#4A0A12 100%);color:#FFFFFF;padding:32px 28px;position:relative}";
  html += ".logo-row{display:flex;align-items:center;gap:12px;margin-bottom:8px}";
  html += ".logo-icon{font-size:28px}";
  html += ".title{font-size:22px;font-weight:800;letter-spacing:-0.5px;color:#FFFFFF}";
  html += ".subtitle{font-size:12px;color:rgba(255,255,255,0.8);text-transform:uppercase;letter-spacing:1px;font-weight:600}";
  html += ".content{padding:28px}";
  html += ".alert-box{background:" + severityBadgeBg + ";border:2px solid " + severityColor + ";border-radius:12px;padding:20px;margin-bottom:24px;text-align:center}";
  html += ".alert-level{font-size:42px;font-weight:900;color:" + severityColor + ";line-height:1}";
  html += ".alert-label{font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:" + severityColor + ";margin-top:6px}";
  html += ".section-title{font-size:14px;font-weight:800;color:#6B0F1A;text-transform:uppercase;letter-spacing:1px;margin:20px 0 10px;padding-bottom:6px;border-bottom:2px solid #F1F5F9}";
  html += ".grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px}";
  html += ".card{background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:12px 14px}";
  html += ".card-label{font-size:11px;font-weight:700;color:#64748B;text-transform:uppercase}";
  html += ".card-val{font-size:14px;font-weight:800;color:#0F172A;margin-top:2px}";
  html += ".warning-banner{background:#FEF2F2;border-left:4px solid #DC2626;border-radius:8px;padding:14px 16px;margin:20px 0;color:#991B1B;font-size:13px;font-weight:600}";
  html += ".footer{background:#F8FAFC;border-top:1px solid #E2E8F0;padding:20px 28px;text-align:center;font-size:12px;color:#64748B}";
  html += ".footer strong{color:#6B0F1A}";
  html += "</style></head><body>";
  html += "<div class='container'>";
  html += "<div class='header'><div class='logo-row'><span class='logo-icon'>🚨</span><div class='title'>EMERGENCY SOS ALERT</div></div><div class='subtitle'>PocketDoctor Mark 4 Autonomous Clinical Triage</div></div>";
  html += "<div class='content'>";
  html += "<div class='alert-box'><div class='alert-level'>" + String(level) + " / 10</div><div class='alert-label'>Emergency Severity Level</div></div>";
  
  html += "<div class='warning-banner'>⚠️ <strong>IMMEDIATE ACTION REQUIRED:</strong> Emergency SOS trigger pressed on PocketDoctor terminal. Please review patient vitals and contact emergency services if necessary.</div>";
  
  html += "<div class='section-title'>👤 Patient Vitals & Medical Profile</div>";
  html += "<div class='grid'>";
  html += "<div class='card'><div class='card-label'>Patient Name</div><div class='card-val'>" + (p.name.length() > 0 ? p.name : "Unregistered") + "</div></div>";
  html += "<div class='card'><div class='card-label'>Age / Gender</div><div class='card-val'>" + String(p.age) + " yrs</div></div>";
  html += "<div class='card'><div class='card-label'>Height / Weight</div><div class='card-val'>" + String(p.height) + " cm / " + String(p.weight) + " kg</div></div>";
  html += "<div class='card'><div class='card-label'>Intoxication</div><div class='card-val'>" + p.intoxStatus + "</div></div>";
  html += "<div class='card'><div class='card-label'>Medical History</div><div class='card-val'>" + String(p.hasMedicalHistory ? "Chronic / Yes" : "None Reported") + "</div></div>";
  html += "<div class='card'><div class='card-label'>Preferred Care</div><div class='card-val'>" + p.treatmentPreference + "</div></div>";
  html += "</div>";

  html += "<div class='section-title'>📡 System & Location Telemetry</div>";
  html += "<div class='grid'>";
  html += "<div class='card'><div class='card-label'>Terminal Local IP</div><div class='card-val'>" + ip + "</div></div>";
  html += "<div class='card'><div class='card-label'>Wi-Fi Access Point</div><div class='card-val'>" + WiFi.SSID() + "</div></div>";
  html += "<div class='card'><div class='card-label'>System Uptime</div><div class='card-val'>" + String(uptimeSecs) + " seconds</div></div>";
  html += "<div class='card'><div class='card-label'>Transmission Protocol</div><div class='card-val'>SSL / SMTP Port 465</div></div>";
  html += "</div>";

  html += "</div>";
  html += "<div class='footer'>Dispatched by <strong>PocketDoctor Mark 4</strong> · Designed & Engineered by <strong>Vaidik Khurana</strong></div>";
  html += "</div></body></html>";
  return html;
}

// -------------------- DIRECT SSL SMTP ENGINE (WiFiClientSecure) --------------------
void dispatchSOSAlert() {
  showLoadingScreen("Sending SOS...", "SSL Connecting");

  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("SOS Failed: WiFi disconnected!");
    display.clearDisplay();
    display.setTextSize(1);
  ...
  String recipientEmail = (wifiConfigValid && strlen(wifiConfig.sosEmail) > 0) ? String(wifiConfig.sosEmail) : String(default_sos_email);
  String smtpHost = (wifiConfigValid && strlen(wifiConfig.smtpServer) > 0) ? String(wifiConfig.smtpServer) : String(default_smtp_server);
  uint16_t smtpPort = (wifiConfigValid && wifiConfig.smtpPort > 0) ? wifiConfig.smtpPort : default_smtp_port;
  String smtpUser = (wifiConfigValid && strlen(wifiConfig.smtpUser) > 0) ? String(wifiConfig.smtpUser) : String(default_smtp_user);
  String smtpPass = (wifiConfigValid && strlen(wifiConfig.smtpPass) > 0) ? String(wifiConfig.smtpPass) : String(default_smtp_pass);
  ...
  // If user enters app pass with spaces, clean spaces
  smtpPass.replace(" ", "");

  if (recipientEmail.length() < 3 || smtpHost.length() < 3) {
    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 20);
    display.println("SOS Failed!");
    display.println("Invalid SMTP Config");
    display.println("Check Portal /sos");
    display.display();
    delay(3000);
    return;
  }
  ...
  client.setInsecure(); // Bypass cert verification on ESP32

  Serial.println("Connecting SSL to SMTP Server: " + smtpHost + ":" + String(smtpPort));
  showLoadingScreen("Connecting SSL...", smtpHost.c_str());
  ...
  if (!client.connect(smtpHost.c_str(), smtpPort)) {
    ...
    display.setCursor(0, 20);
    display.println("SOS Failed!");
    display.println("SSL Conn Error");
    display.display();
    delay(3000);
    return;
  }
  ...
  String response;
  if (!readSmtpResponse(client, "220", response)) {
    Serial.println("SMTP Greeting Failed!");
    client.stop();
    return;
  }

  showLoadingScreen("Authenticating...", "EHLO Handshake");
  client.println("EHLO PocketDoctor.local");
  if (!readSmtpResponse(client, "250", response)) {
    client.println("HELO PocketDoctor.local");
    readSmtpResponse(client, "250", response);
  }

  if (smtpUser.length() > 0 && smtpPass.length() > 0) {
    showLoadingScreen("Authenticating...", "AUTH LOGIN");
    client.println("AUTH LOGIN");
    if (!readSmtpResponse(client, "334", response)) {
      Serial.println("AUTH LOGIN rejected");
      client.stop();
      return;
    }

    client.println(base64Encode(smtpUser));
    if (!readSmtpResponse(client, "334", response)) {
      Serial.println("User Base64 rejected");
      client.stop();
      return;
    }

    client.println(base64Encode(smtpPass));
    if (!readSmtpResponse(client, "235", response)) {
      Serial.println("Password Auth Failed!");
      display.clearDisplay();
      display.setTextSize(1);
      display.setCursor(0, 20);
      display.println("SOS Auth Failed!");
      display.println("Check SMTP Pass");
      display.display();
      client.stop();
      delay(3000);
      return;
  ...
  showLoadingScreen("Sending SOS...", "Addressing Mail");
  String fromAddress = (smtpUser.length() > 0) ? smtpUser : "sos@pocketdoctor.local";
  client.println("MAIL FROM:<" + fromAddress + ">");
  readSmtpResponse(client, "250", response);

  client.println("RCPT TO:<" + recipientEmail + ">");
  readSmtpResponse(client, "250", response);

  client.println("DATA");
  readSmtpResponse(client, "354", response);

  showLoadingScreen("Sending SOS...", "Transmitting Data");

  // Construct MIME HTML email headers and body
  client.println("From: PocketDoctor Emergency <" + fromAddress + ">");
  client.println("To: Emergency Contact <" + recipientEmail + ">");
  client.println("Subject: URGENT SOS ALERT - Severity " + String(sosLevel) + "/10 (PocketDoctor)");
  ...
  String emailHTML = createSOSEmailHTML(sosLevel, currentProfile, WiFi.localIP().toString(), millis() / 1000);
  client.println(emailHTML);

  // End DATA block with CRLF.CRLF
  client.println(".");
  bool sentSuccess = readSmtpResponse(client, "250", response);

  client.println("QUIT");
  readSmtpResponse(client, "221", response);
  client.stop();
  ...
  display.setCursor(0, 20);
  if (sentSuccess) {
    display.println("SOS HTML Sent!");
    display.println("Gmail Delivered");
    display.println("Successfully");
  } else {
    display.println("SOS Send Error!");
    display.println("SMTP Rejected");
  }
  display.display();
  delay(3500);
}
...
void saveSOSConfig(const char* email, const char* server, uint16_t port, const char* user, const char* pass) {
  EEPROM.begin(EEPROM_SIZE);
  Config cfg;
  if (wifiConfigValid) {
    cfg = wifiConfig;
  ...
  cfg.sosEmail[63] = '\0';
  strncpy(cfg.smtpServer, server, 63);
  cfg.smtpServer[63] = '\0';
  cfg.smtpPort = port;
  strncpy(cfg.smtpUser, user, 63);
  cfg.smtpUser[63] = '\0';
  strncpy(cfg.smtpPass, pass, 63);
  cfg.smtpPass[63] = '\0';
  ...
  wifiConfig = cfg;
  wifiConfigValid = true;
  Serial.println("SOS SMTP Settings saved to EEPROM");
}
...
<div class="card-icon">🚨</div>
<div>
<div class="card-label">SOS & SMTP Settings</div>
<div class="card-desc">Configure emergency contact & SMTP email server</div>
</div>
</div>
<span class="arrow-icon">→</span>
</a>
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>SOS & SMTP Settings — PocketDoctor</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<style>
...
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Plus Jakarta Sans',system-ui,sans-serif;background:linear-gradient(135deg,var(--bg-start) 0%,#F5EAEB 50%,var(--bg-end) 100%);color:var(--text);min-height:100vh;padding:24px 16px;line-height:1.6;display:flex;align-items:center;justify-content:center}
.app-window{width:100%;max-width:540px;background:var(--card-bg);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid var(--card-border);border-radius:var(--radius);padding:32px 28px;box-shadow:var(--shadow)}
.header{margin-bottom:24px;border-bottom:1.5px solid rgba(107,15,26,0.08);padding-bottom:16px}
.title{font-size:1.3rem;font-weight:800;color:var(--burg-dark);display:flex;align-items:center;gap:10px}
.subtitle{font-size:0.82rem;color:var(--text-sub);margin-top:4px}
.form-group{margin-bottom:16px}
label{display:block;font-size:0.85rem;font-weight:700;color:var(--burg-dark);margin-bottom:6px}
input[type=text],input[type=email],input[type=number],input[type=password]{width:100%;padding:11px 15px;font-size:0.92rem;font-family:inherit;background:rgba(255,255,255,0.9);border:1.5px solid rgba(232,197,201,0.8);border-radius:12px;color:var(--text);transition:all 0.2s}
input:focus{outline:none;border-color:var(--burg);box-shadow:0 0 0 4px rgba(107,15,26,0.12);background:#fff}
.btn-submit{width:100%;margin-top:10px;padding:14px;font-size:0.98rem;font-weight:700;font-family:inherit;color:#fff;background:linear-gradient(135deg,var(--burg),var(--accent));border:none;border-radius:12px;cursor:pointer;box-shadow:0 6px 20px rgba(107,15,26,0.25);transition:all 0.2s}
.btn-submit:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(107,15,26,0.35)}
...
<div class="app-window">
<div class="header">
<div class="title">🚨 SOS & SMTP Configuration</div>
<div class="subtitle">Configure emergency recipient contact and SMTP credentials</div>
</div>
<form action="/sos/update" method="POST">
<div class="form-group">
<label>SOS Recipient Email</label>
<input type="email" name="email" value="%EMAIL%" required placeholder="e.g. emergency@doctor.com">
</div>
<div class="form-group">
<label>SMTP Server Host</label>
<input type="text" name="server" value="%SERVER%" required placeholder="e.g. smtp.gmail.com">
</div>
<div class="form-group">
<label>SMTP Server Port</label>
<input type="number" name="port" value="%PORT%" required placeholder="e.g. 465 or 587">
</div>
<div class="form-group">
<label>SMTP Account Username</label>
<input type="text" name="user" value="%USER%" placeholder="e.g. username@gmail.com">
</div>
<div class="form-group">
<label>SMTP Account Password</label>
<input type="password" name="pass" value="%PASS%" placeholder="Enter 16-letter App Password">
</div>
<button type="submit" class="btn-submit">Save SOS & SMTP Settings</button>
</form>
<a class="back-link" href="/">← Back to Control Portal</a>
</div>
</body>
...
void handleSOS() {
  String email = wifiConfigValid ? String(wifiConfig.sosEmail) : String(default_sos_email);
  String serverHost = wifiConfigValid ? String(wifiConfig.smtpServer) : String(default_smtp_server);
  uint16_t port = wifiConfigValid ? wifiConfig.smtpPort : default_smtp_port;
  String user = wifiConfigValid ? String(wifiConfig.smtpUser) : String(default_smtp_user);
  String pass = wifiConfigValid ? String(wifiConfig.smtpPass) : String(default_smtp_pass);

  String html = FPSTR(SOS_HTML);
  html.replace("%EMAIL%", email);
  html.replace("%SERVER%", serverHost);
  html.replace("%PORT%", String(port));
...
void handleSOSUpdate() {
  if (server.hasArg("email") && server.hasArg("server") && server.hasArg("port")) {
    ...
    if (email.length() > 0 && serverHost.length() > 0 && port > 0) {
      saveSOSConfig(email.c_str(), serverHost.c_str(), port, user.c_str(), pass.c_str());
      server.sendHeader("Location", "/");
      server.send(303, "text/plain", "");
    } else {
      server.send(400, "text/plain", "Email, Server Host, and Port cannot be empty");
    }
  } else {
    server.send(400, "text/plain", "Missing parameters");
  }
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    isAPMode = true;
    Serial.println("\nWiFi failed/empty, starting SoftAP...");
    WiFi.mode(WIFI_AP);
    WiFi.softAP("PocketDoctor-AP", "12345678");

    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.println("AP Mode Started");
    display.println("SSID: PocketDoctor-AP");
...
void setup() {
  ...
  // Start web server (runs in both STA & AP modes)
  startWebServer();
  ...
  display.setTextSize(1);
  display.println("Doctor v4.8.2.3");
  drawMedicalIcon();
  display.display();
  delay(3000);
...
// ==================== MAIN LOOP ====================
void loop() {
  // Always handle web clients (STA & AP mode)
  server.handleClient();
  ...
  if (inSOSLevelSelection) {
    ...
    if (sosLevel < 10) display.setCursor(52, 32);
    else display.setCursor(44, 32);
